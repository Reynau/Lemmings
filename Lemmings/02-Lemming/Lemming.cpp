...
void Lemming::init(const glm::vec2 &initialPosition, ShaderProgram &shaderProgram, Texture * lemmingTexture)
{
	...
	sprite->setNumberAnimations(24);
		...
		for (int i = 0; i < 8; i++)
			sprite->addKeyframe(LemmingAnims::CLIMBER_LEFT, glm::vec2(float(i) / 10, 596.0f / SPRITE_HEIGHT));
		for (int i = 0; i < 8; i++)
...
Lemming::LemmingState Lemming::getStateFromSkill(LemmingSkill skill) 
{
	...
	if (isGoingLeft()) {
		...
		if (skill == BASHER) return BASHER_LEFT_STATE;
	}
	else {
		...
		if (skill == BASHER) return BASHER_RIGHT_STATE;
	}
}
